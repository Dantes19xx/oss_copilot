...
- ~~**Style-guide personalization**: пользователь загружает свой style-guide (PDF/DOCX) → парсится в RAG → ревью подстраивается под конкретный проект.~~ **Сделано** — `backend/rag/document_ingest.py` (2026-09-16), см. PROGRESS.md. Заодно закрыло отдельный пункт ТЗ 2.2/3.2 про парсинг PDF/DOCX/HTML, который RAG-пайплайн из этапа 4 сам по себе не покрывал (тот индексировал только Markdown из GitHub).
