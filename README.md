...
1. **PR Reviewer** — ревью PR по файлам с человеческим подтверждением перед публикацией комментария в GitHub. Черновик показывает сам diff (что удалено/добавлено) под каждым файлом, а каждое замечание привязано к конкретной строке (`L{номер}` — номер строки в новой версии файла, вычисленный детерминированно по diff'у, а не угадан моделью; см. README ниже).
...
- **Мультимодальность**: `analyze_screenshots` (`backend/graph/vision.py`) находит скриншоты/GIF в описании PR (markdown-синтаксис, `<img>`, известные asset-хосты GitHub) и анализирует их через vision gpt-4o-mini — находки попадают в финальный ревью-комментарий. Как и `review_file()`, кэшируется на уровне графа (`backend/graph/nodes_review.py`, ключ — URL картинки + hash промпта) — повторное ревью того же PR не анализирует одну и ту же картинку заново.
- **Замечания, привязанные к строке**: каждый `ReviewComment` (`backend/graph/schemas.py`) — это цитата конкретной строки diff'а (`code_line`, копируется моделью дословно) + текст замечания. Номер строки не вычисляет модель — это ненадёжно (в тестах модель промахивалась на 1-2 строки даже на простом добавлении файла, и не всегда сохраняла отступы при цитировании). Вместо этого `_new_file_line_map()`/`_resolve_comment_line()` (`backend/graph/nodes_review.py`) детерминированно парсят сам diff и сопоставляют цитату с реальным номером строки в новой версии файла (с fallback-сопоставлением без учёта пробелов). Если цитата не находится в diff'е — замечание помечается как общее ("file"/`general_comment_label`), а не привязывается к случайной строке. `aggregate_review` показывает сам diff (`` ```diff `` блок) под каждым проверенным файлом, а замечания — списком под ним с префиксом `L{номер}`.
...
- **`analyze_image()` результат** (vision) — тот же паттерн на уровне узла `analyze_screenshots`: ключ = URL картинки + hash `VISION_PROMPT`. PR со скриншотом в описании часто ревьюится не один раз — повторный анализ той же картинки не нужен.

Проверено вживую: повторный прогон одного и того же PR — 7 вызовов OpenAI на холодном кэше → 1 вызов (только intake-классификация) на тёплом. Для vision отдельно: первый вызов `analyze_image()` — 2.37с (реальный API-запрос), второй на том же URL — 0.0с, идентичный результат.
