...
**Следующий шаг (этап 17 из PLAN.md раздел 8):** Деплой на публичный URL (Vercel для фронтенда, Railway для backend) — но фронтенда и HTTP-API у графа пока нет (см. этап 13 в PLAN.md, ещё не начат). Прежде чем деплоить, разумно сначала сделать минимальный HTTP-эндпоинт над графом и/или фронтенд — задача для обсуждения с пользователем, куда двигаться дальше: деплоить то, что есть (health-check заглушку) не имеет смысла.
