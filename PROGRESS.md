...
**Следующий шаг (этап 15 из PLAN.md раздел 8):** Docker/docker-compose для backend — уже частично есть (`docker-compose.yml`, `backend/Dockerfile` с этапа 1), нужно проверить актуальность (зависимости с тех пор менялись) и добавить Qdrant + backend вместе в один рабочий compose-запуск. Следующий по приоритету рекомендуемый модуль (PLAN.md раздел 3).
