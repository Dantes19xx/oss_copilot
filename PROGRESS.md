...
**Важно (для следующей сессии / деплоя):** флаг — билд-тайм, значит **не переключается без пересборки**. На Vercel сейчас `NEXT_PUBLIC_SHOW_EVAL_STATS` не задан (полоса выключена по умолчанию) — если понадобится показать на защите, нужно `vercel env add NEXT_PUBLIC_SHOW_EVAL_STATS production` со значением `true` и передеплоить, не просто поменять переменную "на лету".
