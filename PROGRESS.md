...
- `intake()` и `vision.analyze_image()` НЕ кэшируются — осознанно не стал: intake обрабатывает произвольный текст пользователя (низкая вероятность точного повтора), vision — URL картинок, где кэш по URL was in-scope но не сделан в этом этапе (возможное будущее расширение, не входит в обязательный/рекомендуемый чек-лист сверх уже сделанного).

**Следующий шаг (этап 14 из PLAN.md раздел 8):** Fallback-стратегия между моделями (если gpt-4o-mini недоступен/лимит) — следующий по приоритету рекомендуемый модуль (PLAN.md раздел 3).
