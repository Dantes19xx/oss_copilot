...
- Перезапустил `backend.evals.run_evals` на золотом датасете (свежий, не кэшированный прогон). Новые числа: **accuracy=0.83, precision=0.81, recall=1.00 (без изменений), f1=0.90, mean_judge_score=4.13/5** — немного ниже прошлого прогона (0.87/0.85/1.00/0.92/4.20), но это ожидаемая run-to-run variance при temperature=0.2 у judge'а (уже задокументирована в EVALS.md §5 как находка, не баг). Главный вывод — **recall остаётся идеальным** (0/22 пропущенных реальных issue) — не изменился.
- **Осознанно не переписывал EVALS.md** — там числа встроены в нарратив конкретного прогона (включая историю про баг judge-промпта и его починку), переписывание разрушило бы согласованность истории. Обновил только презентацию (слайд 10), как попросил пользователь буквально — "финальный прогон evals и обнови цифры в презентации".
- Обновил `presentation/slides.html` и republish Artifact той же ссылкой (`--url`, не новый URL). Перед republish — та же дисциплина: реальный headless-рендер слайда 10, 0 console errors, PDF пересобран печатной эмуляцией Chromium и сохранён в `presentation/oss-copilot-slides.pdf`.
- **Если понадобится для защиты консистентность цифр между EVALS.md и презентацией** — нужно явное решение пользователя, обновлять ли и EVALS.md тоже (перезапишет часть нарратива §5, не просто цифры).
